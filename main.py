...
def main() -> None:
    ...
    if st.session_state.edit_mode:
        ...
        with col1:
            with stylable_container(
                key="edit_button_styling",
                css_styles="""button{min-width: 100%;}"""
            ):
        ...
        with col2:
            with stylable_container(
                ...
                # No disabling because of ordering
    ...
    with download_placeholder:
        df_download = pd.DataFrame(grid_response['data'])
        df_download.drop("index",axis=1,inplace=True)
        st.session_state.csv_data=df_download.to_csv(index=False).encode('utf-8')
    ...
    # Post grid button event and setup handling
    if "deletion_flagged" in st.session_state and st.session_state["deletion_flagged"] == True:
    ...
    if "saving_flagged" in st.session_state and st.session_state["saving_flagged"] == True:
        session = get_snowflake_session()
    ...
    # st.session_state["selected_rows"] = [] if type(selected_rows) == type(None) else selected_rows
